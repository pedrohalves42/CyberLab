...
mkdir -p \
  "$BASE/core" \
  "$BASE/governance" \
  "$BASE/modules/governance" \
  "$BASE/operations" \
  "$BASE/state" \
  "$BASE/audit" \
  "$BASE/logs"

# Backup seguro do dispatcher
[ -f "$BASE/bin/cyberlab" ] && cp "$BASE/bin/cyberlab" "$BASE/bin/cyberlab.bak.gov.$(date +%Y%m%d_%H%M%S)"

cat > "$BASE/modules/governance/governance.sh" <<'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x "$BASE/modules/governance/governance.sh"

# Integrar no dispatcher sem recriar o core
python3 <<'PY'
