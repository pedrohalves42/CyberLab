...
# Backup
[ -f "$BASE/bin/cyberlab" ] && cp "$BASE/bin/cyberlab" "$BASE/bin/cyberlab.bak.report.$(date +%Y%m%d_%H%M%S)"
[ -f "$BASE/core/delivery.sh" ] && cp "$BASE/core/delivery.sh" "$BASE/core/delivery.sh.bak.report.$(date +%Y%m%d_%H%M%S)"

cat > "$BASE/modules/report/report-engine.sh" <<'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x "$BASE/modules/report/report-engine.sh"

# Patch seguro do dispatcher
python3 <<'PY'
from pathlib import Path
