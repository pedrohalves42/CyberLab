...
mkdir -p "$BASE/modules/report" "$BASE/state/reports"

cat > "$BASE/modules/report/report-engine-enterprise.sh" <<'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x "$BASE/modules/report/report-engine-enterprise.sh"
...
if "report)" in s:
    ...
    if end != -1:
        s = s[:start] + report_block + s[end+2:]
else:
    idx = s.rfind("*)")
    s = s[:idx] + report_block + "\n" + s[idx:] if idx != -1 else s + "\n" + report_block
...
echo "[OK] Report + Delivery Enterprise instalado"
