...
class Block12Engine:
    ...
    def _cap_surface_noise(self, findings: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Reduz ruído de superfície para relatórios finais.
        Mantém riscos reais sem limite e limita categorias repetitivas como CDN, token e API.
        """
        limits = {
            "SURFACE_CDN": 20,
            "SURFACE_TOKEN": 20,
        ...
        for item in findings:
            ...
            # Nunca limita achado de risco real.
            if category not in SURFACE_CATEGORIES:
                filtered.append(item)
                continue

            limit = limits.get(category, 15)
    ...
    def analyze(self, target_dir: str, target_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        findings = self._cap_surface_noise(findings)
