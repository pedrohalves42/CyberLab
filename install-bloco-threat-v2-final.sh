...
mkdir -p "$BASE/modules/threat" "$BASE/results/threat" "$BASE/state/intelligence"

cat > "$BASE/modules/threat/threat-engine-v2.sh" <<'SCRIPT'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x "$BASE/modules/threat/threat-engine-v2.sh"
...
echo "[OK] Threat Engine V2 instalado"
